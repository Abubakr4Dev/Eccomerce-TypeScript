import { ICartProductModel } from '../interfaces/cart-product.interface';
import { IAddProductToCart, ICartModel, ICreateCartProduct, IRemoveProductFromCart, IUpdateCartProductQuantity } from '../interfaces/cart.interface';
...
export class CartService {
    constructor(
        public cartModel: ICartModel,
        public cartProductModel: ICartProductModel
    ) {}
    ...
    async createCartProduct(createCartProductDto: ICreateCartProduct) {
    ...
    async updateProductQuantity(updateCartProductQuantityDto: IUpdateCartProductQuantity) {
        ...
        const updatedCartProduct = await this.cartProductModel.findOneAndUpdate({ _id: cartProduct._id }, { $inc: { quantity: inc ? amount : -amount } }, { new: true }).populate('product');

        const newPrice = inc ? updatedCartProduct!.product.price * amount : -(updatedCartProduct!.product.price * amount);
        return await this.cartModel.findOneAndUpdate({ _id: cartId }, { $inc: { totalPrice: newPrice } }, { new: true });
    }

    async addProduct(addProductToCartDto: IAddProductToCart, product: IProductDoc) {
        ...
        return await this.cartModel.findOneAndUpdate({ _id: cart._id }, { $push: { products: cartProduct }, $inc: { totalPrice: product.price * quantity } }, { new: true });
    }
}
